...
gold <- mutate(gold, type = "gold")
gold <- mutate(gold, diff = abs(Value - lag(Value)))
gold <- mutate(gold, date_copy = Date)
gold <- separate(gold, col= date_copy, into=c("year", "Month", "Day"), sep = "-" )

gold <- filter(gold, as.numeric(year) >= 2010)

bitcoin <- Quandl("BAVERAGE/USD") %>% tbl_df()

bitcoin <- rename(bitcoin, Value = `24h Average`)
bitcoin <- mutate(bitcoin, type = "bitcoin")
bitcoin <- mutate(bitcoin, diff = abs(Value - lag(Value)))
...
milk <- mutate(milk, Value = Value/50)
